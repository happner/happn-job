ideControllers.controller('flow_new', ['$scope', '$modalInstance', 'data', function($scope, $modalInstance, data) {

	  $scope.data = data;  
	  $scope.message = {type:'alert-warning', message:'', display:'none'};
	  $scope.flow = {name:'', description:'', project:'', type: 'flow', editable:true}; 
...
ideControllers.controller('flow_edit', ['$scope', function($scope) {
	 ...
	 $scope.onDrop = function($event, $data){
	 	...
	 	var shape = {id:shapeId.replace(/ /g,''), 
			 label:shapeId, 
			 icon:'', 
			 sourceEndPoints:[], 
			 targetEndPoints:[], 
	 	...
	 	$scope.drawing.shapes.push(shape);
	 	$scope.$apply();
	 ...
	 $scope.drawingEvent = function(event, params){
		...
		if (event == "connectionDragStop"){
			...
			var connection = {uuids:[fromEndPoint.elementId + fromEndPoint.type, toEndPoint.elementId + toEndPoint.type], editable:true};
			console.log(connection);
			//$scope.flow.drawing.connections.push();
